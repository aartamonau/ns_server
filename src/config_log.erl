...
handle_info({config_change, KV}, State) ->
    {noreply, log_kv(KV, State), hibernate};
...
log_kv({"/buckets", {RawBuckets, Version}}, #state{buckets=OldBuckets} = State) ->
    ...
    log_common(buckets, {BucketsDiff, Version}),
    NewState;
log_kv({K, V}, State) ->
    log_common(K, V),
    State.
...
log_common(K, V) ->
    ...
    ?log_debug("config change:~n~s ->~n~s", [K, VB]).
