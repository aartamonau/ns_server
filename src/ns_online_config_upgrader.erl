...
start_link() ->
    proc_lib:start_link(erlang, apply, [fun do_upgrade_config/0, []]).

do_upgrade_config() ->
    ok = ns_config:upgrade_config_explicitly(fun upgrade_config/1),
    proc_lib:init_ack(ignore).

-spec upgrade_config([[{term(), term()}]]) -> [{set, term(), term()}].
upgrade_config(Config) ->
    case ns_config:search_node(Config, global_config_version) of
        false ->
            [{set, global_config_version, {2,0}} |
